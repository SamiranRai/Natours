...
exports.verifyTheEmailCode = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(new AppError('The Code is not correct or Time is over resent again'));
    }
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!email || !password) return next(new AppError('Please provide your email or password', 400));
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    const resetUrl = `${req.protocol}//:${req.get('host')}/api/v1/users/resetToken/${resetToken}`;
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!await user.correctPassword(req.body.currentPassword, user.password)) {
        return next(new AppError("Current password is wrong! Please enter Right Password...", 400));
    }
